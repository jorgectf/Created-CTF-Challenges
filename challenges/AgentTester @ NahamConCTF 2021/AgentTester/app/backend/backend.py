...
app.config["SQLALCHEMY_DATABASE_URI"] = "mysql+pymysql://ctfuser:ctfpass@mysql/ctfdb"
...
def create_tables():
    ...
    try:
        user = User(
            ...
            password=bcrypt.hashpw(os.environ.get("ADMIN_BOT_PASSWORD").encode(), current_salt).decode(),
            about="",
        )
        db.session.add(user)
        db.session.commit()
...
def signup():
    ...
    if request.method == "GET":
        ...
        user = User(username=username, email=email, 
            password=bcrypt.hashpw(password.encode(), current_salt).decode(), 
            about="",
        )
        db.session.add(user)
        db.session.commit()
...
def signin():
    ...
    if request.method == "GET":
        ...
        return render_template("backend/default_templates/signin.html", success=success, error=error)
        ...
        user = User.query.filter(
            User.username == username, User.password == bcrypt.hashpw(password.encode(), current_salt).decode()
        ).first()
        if not user:
